(function () {
  ...
  // ============================================================
  // Generate 90 days of fake guest data with upward trend
  var allDailyGuests = [];
  var allDailyLabels = [];
  var today = new Date();
  for (var i = 89; i >= 0; i--) {
  ...
  function renderLineChart(range) {
    var labels, data;
    if (range === '7d') {
      labels = allDailyLabels.slice(-7);
      data = allDailyGuests.slice(-7);
    } else if (range === '30d') {
      labels = allDailyLabels.slice(-30);
      data = allDailyGuests.slice(-30);
    } else {
